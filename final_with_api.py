...
def predict_via_api(user_idx, recipe_idx):
    url = "http://localhost:8502/v1/models/my_model:predict"

    # Prepare input in the format expected by your model
    data = {
        "signature_name": "serving_default",
        "instances": [
            {
                "input_layer": [user_idx],     # change names if your model has different inputs
                "input_layer_1": [recipe_idx]
            }
        ]
    }
    ...
    for category in df['category'].unique():
        ...
        for rid in cb_candidates['id']:
            if rid in recipe_to_index:
                ...
                pred= predict_via_api(user_idx, r_idx)
